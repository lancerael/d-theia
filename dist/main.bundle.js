...
var Axis = __webpack_require__(4);
...
var AxisChart = function (_Chart) {
  ...
  */
  function AxisChart() {
    ...
    _this.jPadding = { l: 35, r: 15, t: 25, b: 90 };
  ...
  _createClass(AxisChart, [{
    ...
    value: function setDimensions() {
      ...
      this.oScaleX.domain(this.aData.map(function (d) {
        return d[_this2.jConfig.aAxisKeys[0]];
      })).range([0, this.iInnerWidth]);
      this.oScaleY.domain([0, d3.max(this.aData, function (d) {
        var aValues = [];
        _this2.jConfig.aValues.forEach(function (oValue) {
...
var Chart = function () {
  ...
  */
  function Chart() {
    ...
    var jConfig = oParams.jConfig,
        aData = oParams.aData,
        sContainer = oParams.sContainer;
    var oContainer = oParams.oContainer;
  ...
  */
  ...
  _createClass(Chart, [{
    key: 'setContainer',
    value: function setContainer(oContainer) {
    ...
    value: function init() {
      ...
      if (this.aData && this.jConfig && !isNaN(this.iWidth) && !isNaN(this.iHeight)) {
        ...
        this.oResizeWatcher = this.oResizeWatcher || window.addEventListener('resize', function () {
          _this.setDimensions();
          _this.iResizeOffset = _this.iWidth - _this.iInitialWidth;
          if (_this.renderChart) {
            _this.renderChart();
          }
          _this.oToolTip.hide();
        });
...
var Key = function () {
  ...
  */
  function Key(_ref) {
    var oContainer = _ref.oContainer,
  ...
  */
  ...
  _createClass(Key, [{
    ...
    value: function render() {
      ...
      var iGroupOffset = 0;
      var oKeyG = this.oContainer.append('g').attr('class', 'key');
      var aLabels = oKeyG.selectAll('text.label').data(this.aValues);
      var calculateMargin = function calculateMargin(aValues, i) {
        var iMargin = 0;
        if (i) {
          for (var l = 0; l < i; l++) {
      ...
      aLabels.enter().append('text').text(function (d) {
        return d.sName;
        ...
        return calculateMargin(_this.aValues, i);
      }).attr('y', 8).attr('width', 10).attr('height', 10).attr('font-family', 'sans-serif').attr('font-size', '10px').attr('fill', '#222222');
      var aKeys = oKeyG.selectAll('rect.key').data(this.aValues);
      aKeys.enter().append('rect').attr('class', 'key').attr('fill', function (d) {
        return d.sColour;
      }).attr('x', function (d, i) {
        return calculateMargin(_this.aValues, i) - 12;
  ...
  _createClass(Axis, [{
    ...
    */
    ...
    value: function renderAxisX() {
      ...
      this.oContainer.append('g').attr('class', 'x-axis').call(d3.axisBottom(this.oScaleX)).attr('transform', 'translate(' + this.jPadding.l + ',' + this.iHeight + ')').selectAll('text').attr('x', -5).attr('y', 6).attr('transform', 'rotate(310)').attr('class', 'x-labels').text(function (d) {
        return Utilities.truncateString(d, _this.iTruncate);
      }).style('text-anchor', 'end').on('mousemove', function (d) {
        if (_this.oToolTip && d.length > _this.iTruncate) {
          _this.oToolTip.ping(d3.event.clientX + 8, d3.event.clientY + 5 + document.body.scrollTop, '<strong>' + d + '</strong>');
        }
      });
    }
    ...
    */
    ...
    value: function renderAxisY() {
      ...
      this.oContainer.append('g').attr('class', 'y-axis').call(d3.axisLeft(this.oScaleY)).attr('transform', 'translate(' + this.jPadding.l + ',0)').selectAll('.y-axis .tick line').attr('x2', function () {
    ...
    */
    ...
    value: function renderLabels() {
      ...
      this.oContainer.append('text').attr('class', 'labels').attr('x', this.iHeight / -2 - this.jPadding.t / 2) // - ((this.jConfig.aAxisLabels[0].length / 2) * 7))
      .attr('y', 10).attr('transform', 'rotate(-90)').attr('text-anchor', 'middle').text(this.aAxisLabels[0]);
      this.oContainer.append('text').attr('class', 'labels').attr('x', (this.iWidth + this.jPadding.l + this.jPadding.r) / 2) // - ((this.aAxisLabels[1].length / 2) * 2))
...
var Utilities = {
  ...
  */
  getPromiseJSON: function getPromiseJSON(sUrl) {
  ...
  */
  truncateString: function truncateString(sString, iMaxLength) {
  ...
  */
  generateRandomInteger: function generateRandomInteger(iMin, iMax, iOmit) {
  ...
  */
  sliceSampleData: function sliceSampleData(aData) {
    var iMaxLength = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 50;

    var iStart = this.generateRandomInteger(0, aData.length - iMaxLength);
    var iEnd = iStart + this.generateRandomInteger(3, iMaxLength);
...
var Theia = __webpack_require__(7);
var Utilities = __webpack_require__(5);
...
Utilities.getPromiseJSON('./data/formula1.json').then(function (data) {
  Theia.createBubbleChart({
    ...
    aData: function () {
      ...
      try {
        for (var _iterator = data.MRData.RaceTable.Races[0].Results[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var jItem = _step.value;
...
var BarChart = __webpack_require__(8);
var LineChart = __webpack_require__(11);
var BubbleChart = __webpack_require__(12);
...
var Theia = {
  ...
  */
  createBarChart: function createBarChart(oParams) {
    var oBarChart = new BarChart(oParams).init();
  ...
  * @param {Object} oParams includes target DOM object, JSON config and array of data
  */
  createLineChart: function createLineChart(jParams) {
    var oLineChart = new LineChart(jParams).init();
  ...
  * @param {Object} oParams includes target DOM object, JSON config and array of data
  */
  createBubbleChart: function createBubbleChart(jParams) {
    var oBubbleChart = new BubbleChart(jParams).init();
...
var d3 = __webpack_require__(0);
var AxisChart = __webpack_require__(1);
  ...
  _createClass(BarChart, [{
    ...
    */
    value: function renderChart(bReset) {
      ...
      var _jConfig = this.jConfig,
          aAxisKeys = _jConfig.aAxisKeys,
          aValues = _jConfig.aValues,
          sBarType = _jConfig.sBarType;
      ...
      aValues.forEach(function (oValues, i) {
        var sKey = oValues.sKey,
            sColour = oValues.sColour;

        var iBarOffset = sBarType === 'side' ? iBarWidth * i : 0;
        // Reset bars data and clear graph
        if (bReset) {
          _this2.aBars[i] = _this2.oChartG.selectAll('rect.bars').data({});
        ...
        if (!_this2.aBars[i]) {
          // Bind bars data
          _this2.aBars[i] = _this2.oChartG.selectAll('rect.bars-' + i).data(_this2.aData);
          // Add new rect elements and set base attributes
          _this2.aBars[i].enter().append('rect').on('mousemove', function (d) {
            _this2.oToolTip.ping(d3.event.clientX, d3.event.clientY + 20, '<strong>' + d[aAxisKeys[0]] + '</strong><br>' + oValues.sName + ': <em>' + d[oValues.sKey] + '</em>');
            ...
            return _this2.oToolTip.hide();
          }).attr('class', 'bars bars-' + i).attr('fill', sColour).attr('y', iInnerHeight).attr('height', 0);
        }

        // Update bars
        _this2.oChartG.selectAll('rect.bars-' + i).attr('x', function (d) {
          return oScaleX(d[aAxisKeys[0]]) + iBarOffset;
        }).attr('width', iBarWidth).transition().ease(d3.easeLinear).duration(_this2.iTransitionTime).attr('y', function (d) {
          return oScaleY(d[sKey]);
        }).attr('height', function (d) {
          return iInnerHeight - oScaleY(d[sKey]);
        });
      });
    }
  }]);
  ...
  */
  ...
  _createClass(Tooltip, [{
    ...
    */
    ...
    key: 'ping',
    value: function ping(x, y, sContent) {
      var _this = this;

      var iPageOffsetY = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
    ...
    */
    value: function renderChart() {
      ...
      aValues.forEach(function (oValues, i) {
        ...
        _this2.aLines[i] = d3.line().x(function (d) {
          return oScaleX(d[aAxisKeys[0]]) + oScaleX.bandwidth() / 2;
        }).y(function (d) {
          return oScaleY(d[sKey]);
        });

        // Add the valueline path.
        _this2.oChartG.append('path').data([_this2.aData]).attr('class', 'line').attr('stroke', sColour).attr('d', _this2.aLines[i]);
      });
    }
  }]);
    ...
    */
    ...
    value: function renderChart() {
      ...
      var fnTicked = function fnTicked() {
        _this3.oBubblesG.selectAll('circle').attr('cx', function (d) {
          return d.x;
        }).attr('cy', function (d) {
          return d.y;
        });
      ...
      if (!this.aCircles) {
        ...
        // Add new rect elements and set base attributes
        this.aCircles.enter().append('circle').on('mousemove', function (d) {
          _this3.oToolTip.ping(d3.event.clientX, d3.event.clientY + 20, '<strong>' + d[aAxisKeys[0]] + '</strong><br>' + sName1 + ': <em>' + d[sKey1] + '</em><br>' + sName2 + ': <em>' + d[sKey2] + '</em>');
        }).on('mouseout', function () {
