...
export default class Colors {
  ...
  static getRandomColor(sBackground = 'light', iBoundary = 170) {
    ...
    while (
      !aValues.length ||
      !this.colorFilter(sBackground, iBoundary, aValues)
    ) {
   ...
   */
  static colorFilter(sBackground: string, iBoundary: number, aValues: any[]) {
    let bPass = false
    if (!aValues.length) {
      bPass = false
    } else if (!sBackground) {
      ...
      const iBrightness =
        (aValues[0] * 299 + aValues[1] * 587 + aValues[2] * 114) / 1000
   ...
   */
  static convertHexToRgb(sColor: string) {
    return rgb(sColor)
  }
   ...
   */
  static getDarkerColor(oColor: any) {
